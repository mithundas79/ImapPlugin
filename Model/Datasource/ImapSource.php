...
class ImapSource extends DataSource {
     ...
     */
    public $description = 'imap datasource';
     ...
     * Model $model, $queryData = Array, $recursive = NULL
     */
    public function read(Model $Model, $query = array(), $recursive = null) {
        ...
        if ($uids === false) {
            ...
            $uids = imap_sort(
                    $this->Stream, $orderCriteria, $orderReverse, SE_UID, join(' ', $searchCriteria)
            );
        }
        ...
        return $this->err(
                        $Model, 'Unknown find type %s for query %s', $Model->findQueryType, $query
        );
    }
    ...
    public function connect($Model, $query) {
        ...
        switch ($this->config['type']) {
            case 'imap':
                $this->_connectionString = sprintf(
                        '{%s:%s%s%s}%s', $this->config['server'], $this->config['port'], @$this->config['ssl'] ? '/ssl' : '', @$this->config['connect'] ? '/' . @$this->config['connect'] : '', @$this->config['tail'] ? @$this->config['tail'] : ''
                );
                break;

            case 'pop3':
                $this->_connectionString = sprintf(
                        '{%s:%s/pop3%s%s}', $this->config['server'], $this->config['port'], @$this->config['ssl'] ? '/ssl' : '', @$this->config['connect'] ? '/' . @$this->config['connect'] : ''
                );
                break;
        }
        ...
        try {
            ...
            if (!$this->thread) {
                return $this->err(
                                $Model, 'Unable to get imap_thread after %s retries. %s', $retries, imap_last_error()
                );
            }
        } catch (Exception $Exception) {
            return $this->err(
                            $Model, 'Unable to get imap_thread after %s retries. %s', $retries, $Exception->getMessage() . ' ' . imap_last_error()
            );
        }
    ...
    protected function _getFormattedMail($Model, $uid, $fetchAttachments = false) {
        ...
        if ($uid != ($mailuid = imap_uid($this->Stream, $msg_number))) {
            pr(compact('Mail'));
            return $this->err(
                            $Model, 'Mail id mismatch. parameter id: %s vs mail id: %s', $uid, $mailuid
            );
        }
        ...
        if (!($Mail = imap_headerinfo($this->Stream, $msg_number)) || !property_exists($Mail, 'date')) {
            pr(compact('Mail'));
            return $this->err(
                            $Model, 'Unable to find mail date property in Mail corresponding with uid: %s. Something must be wrong', $uid
            );
        }
        ...
        if (!($flatStructure = $this->_flatStructure($Model, $uid))) {
            return $this->err(
                            $Model, 'Unable to find structure type property in Mail corresponding with uid: %s. Something must be wrong', $uid
            );
        }
        ...
        if (!empty($Structure->parts)) {
            foreach ($Structure->parts as $n => $Part) {
                ...
                if (!empty($Part->parts)) {
                    if ($Part->type == 1) {
                        $flatParts = Set::merge(
                                        $flatParts, $this->_flatStructure($Model, $uid, $Part, $partnr . '.' . ($n + 1))
                        );
                    } else {
                        foreach ($Part->parts as $idx => $Part2) {
                            $flatParts = Set::merge(
                                            $flatParts, $this->_flatStructure($Model, $uid, $Part2, $partnr . '.' . ($idx + 1))
                            );
                        }
                    }
                }
